...
def suite():
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='age357-ownership-') as tmp:
            ...
            for mode in ['normal-held', 'ready-root-exit', 'lock-busy', 'cancel-complete', 'owner-cancel', 'loss-held', 'worker-loss', 'acquisition-loss']:
                d = root / mode; d.mkdir()
                ...
                if mode == 'owner-cancel':
                    ...
                    if mode in ['normal-held', 'worker-loss', 'lock-busy']: args += ['--completion-scope', 'root']
                ...
                elif mode == 'worker-loss':
                    os.kill(transfer, signal.SIGKILL)
                if mode not in ['owner-cancel', 'worker-loss']:
                    ...
                    time.sleep(.4)
                    if mode != 'loss-held': assert Path(f"/proc/{identity['owner']}").exists()
                ...
                if mode in ['owner-cancel', 'worker-loss']:
